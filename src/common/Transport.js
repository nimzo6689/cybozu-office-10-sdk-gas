...
export default class CybozuTransport extends LoadReductionCallable {
  ...
  constructor(baseUrl, accountId, password, sleepSec) {
    ...
    this._session =
      JSON.parse(
        CacheService.getScriptCache().get(CybozuTransport.CYBOZU_SESSION_KEY)
      ) || {};
  ...
  _renewLoginSession(accountId, password) {
    ...
    this._session.cookie = Utils.createCookieValue(
      /AGSESSID=(.*?);/i.exec(headers["Set-Cookie"].toString())[1]
    );

    // csrfTicketを取得するため、TOPページにアクセスする。
    const pageCsrfTicket = this.get();
    this._session.csrfTicket =
      /<input type="hidden" name="csrf_ticket" value="(.*?)">/i.exec(
        pageCsrfTicket
      )[1];
    CacheService.getScriptCache().put(
      CybozuTransport.CYBOZU_SESSION_KEY,
